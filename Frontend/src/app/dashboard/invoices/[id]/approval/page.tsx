...
export default function InvoiceApprovalPage() {
  ...
  const handleAddComment = (e: FormEvent) => {
    ...
    addComment(invoice.id, comment.trim());
    setInvoice(getInvoice(invoice.id) ?? null);
    setComment("");
  };

  const isDecided = invoice.status === "approved" || invoice.status === "rejected";
            ...
            <div className="flex items-center gap-xs">
              <span
                className={`w-2 h-2 rounded-full ${
                  invoice.priority === "High" ? "bg-error" : "bg-primary"
                }`}
              />
              <span className="font-title-md text-title-md text-on-surface">
                {invoice.priority}
            ...
            <div className="flex justify-between items-start mb-md">
              ...
              <div className="bg-primary/10 px-md py-1 rounded-full">
                <span className="font-label-sm text-label-sm text-primary uppercase">
                  {isDecided ? invoice.status : "Pending"}
                </span>
              </div>
            </div>
          ...
          <div className="bg-surface-container-low p-lg rounded-xl shadow-sm">
            <div className="flex flex-col gap-lg relative">
              ...
              {invoice.auditTrail.map((event) => (
                <div key={event.id} className="flex gap-md relative">
          ...
          {isDecided ? (
            ...
            >
              <span className="material-symbols-outlined">
                {invoice.status === "approved" ? "check_circle" : "cancel"}
              </span>
